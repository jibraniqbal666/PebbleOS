...
void cron_service_handle_clock_change(PebbleSetTimeEvent *set_time_info) {
  ...
  while (s_scheduled_jobs != NULL) {
    ...
    PBL_LOG(LOG_LEVEL_INFO, "Cron job rescheduled for %lld", (long long)job->cached_execute_time);
...
time_t cron_job_schedule(CronJob *job) {
  ...
  PBL_LOG(LOG_LEVEL_DEBUG, "Cron job scheduled for %lld (%+lld)", (long long)job->cached_execute_time,
          (long long)(job->cached_execute_time - now));
...
time_t cron_job_schedule_after(CronJob *job, CronJob *new_job) {
  ...
  *new_job = temp_job;
  ...
  PBL_LOG(LOG_LEVEL_DEBUG, "Cron job scheduled for %lld", (long long)job->cached_execute_time);
