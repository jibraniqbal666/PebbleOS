...
static MfgData prv_fetch_struct(void) {
  ...
  if (result.data_version != CURRENT_DATA_VERSION) {
      ...
      result.color = WATCH_INFO_COLOR_COREDEVICES_CT2_BLACK; // Default color
      strncpy(result.model, "ESP32C3", sizeof(result.model));
...
bool mfg_info_is_hrm_present(void) {
  return false; // ESP32-C3 doesn't have HRM by default
}
